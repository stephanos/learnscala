...
import com.loops101.util.{TimeUtil, PropUtil, SystemUtil, LogUtil}
...
trait MailUtil {
  ...
  class MailUtilImpl {
    ...
    def sendMail(from: String, to: List[String], replyTo: Option[String],
      ...
      html match {
        case Some(h) =>
          ...
          mail.setSentDate(TimeUtil.now.toDate)
    ...
    private def getEnv(name: String) =
      sysUtil.getEnvProperty(name).getOrElse(null)
  ...
  lazy val mailUtil = new MailUtilImpl
}
